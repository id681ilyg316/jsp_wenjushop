										...
										<c:forEach items="${sessionScope.shoppingCart.cartItems }" var="cartItem">
											<tr>
												<td class="product-thumbnail"><div>
														<img src="${pageContext.request.contextPath }${cartItem.wenju.cover}"
															style="width: 100px; height: 150px">
													</div></td>
												<td class="product-name">${cartItem.wenju.name }</td>
												<td class="product-price">${cartItem.wenju.price }元</td>
												<td>${cartItem.wenju.stock }</td>
												<c:choose>
													<c:when test="${cartItem.wenju.stock <  cartItem.quantity}">
														<td class="product-quantity"><input type="number" value="${cartItem.wenju.stock}"
															onMouseOut="updateCart(this,${cartItem.wenju.id },${cartItem.wenju.stock })" /></td>
													</c:when>
													<c:otherwise>
														<td class="product-quantity"><input type="number" value="${cartItem.quantity }"
															onMouseOut="updateCart(this,${cartItem.wenju.id },${cartItem.quantity })" /></td>

													</c:otherwise>
												</c:choose>
												<td class="product-remove"><a
													href="${pageContext.request.contextPath }/CartServlet?method=delete&id=${cartItem.wenju.id}"><i
