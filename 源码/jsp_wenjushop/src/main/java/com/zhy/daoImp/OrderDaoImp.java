...
public class OrderDaoImp implements OrderDao {
	...
	public void add(ShoppingCart shoppingCart, String user,String shou,String address,String info,String phone) throws Exception {
		...
		for (CartItem cartItem : shoppingCart.getCartItems()) {
			order = new Order(cartItem.getWenju().getName(), cartItem.getWenju().getPrice(), cartItem.getQuantity(),
					(int) cartItem.getSubtotalPrice(), user, cartItem.getWenju().getId(), shou, address, info, phone,0);
			orders.add(order);
		}
		String sql = null;
		Connection connection = DBUtil.getConnection();
		...
		for (Order order2 : orders) {
			sql = "insert orders (name, price, quantity, total, user, wenjuid,shou,address,info,phone,status)" + "value(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
			...
			preparedStatement.setInt(6, order2.getWenjuid());
